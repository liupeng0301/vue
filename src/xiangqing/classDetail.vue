<template>
	<div class="darkContainer">
		...
		<second-Header msgHeader="课程详情" :showIcon="true"></second-Header>
		...
		<!-- <mt-swipe :auto="4000" class="swipe">
			<mt-swipe-item v-for="(img,index) in cimg" :key="img" style="background-color: #fff">
				<img ref="bannerHeight" :src="img" alt="" style="width: 100%;" @click="clickImg(index)">
			</mt-swipe-item>
		</mt-swipe> -->
		<div class="swiper-container kecheng">
			<div class="swiper-wrapper">
				<div class="swiper-slide zoomImage2" v-for="(img,index) in cimg" :key="img" style="background-color: #fff" :style="getImge(img)">
				</div>
			</div>
		...
		<section class="item-name">
			...
			<p class="desc"><span>{{item.teacherName}}</span><span>{{item.courseNum?item.courseNum:0}}</span><span>课程人数上限:{{item.manNumber?item.manNumber:0}}人</span></p>
		</section>

		<section class="images">
			<p class="tit2">图文详情</p>
			<div class="detail-discribe">
				<img v-for="(i,index) in item.picTextDetailUrlList" :key="index" :src="i" alt="" @click="clickDet(index)">
			</div>
		</section>
		...
		<footer v-else>
			...
			<button @click="choice()">选择课程</button>
		</footer>
		<!-- 	<footer>
			<button @click="service">
				<img src="../assets/icons/service.png" alt="" class="detailFootIcon">
...
<script>
	...
	export default {
		...
		created() {
			...
			this.$axios({
					method: 'post',
					url: this.$url + 'course/json/getCourseDetail',
					data: postData,
					headers: {
		...
		methods: {
			swi() {
				const _this = this;
				if (this.cimg.length == 1) { //一个不需要循环
					// var myswiper=new Swiper('.swiper-container',{})
					var mySwiper = new Swiper('.kecheng', {
						loop: false,
						observer: false, //修改swiper自己或子元素时，自动初始化swiper
						observeParents: false, //修改swiper的父元素时，自动初始化swiper
						autoplayDisableOnInteraction: false, //用户滑动后是否自动播放
			...
			choice() {
				if (this.$route.params.userid == "type=true") {
					...
					this.$router.push(`/classList/${this.userId}/${this.item.id}?status=1`)
				}
